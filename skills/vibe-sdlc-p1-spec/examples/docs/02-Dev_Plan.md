...
| 階段 | H-Director 職責 | H-Reviewer 職責 |
|------|-----------------|-----------------|
| **規格定義** | 撰寫/定案 PRD、SRD、API Spec | 交叉審查規格一致性 |
| **任務分派** | 核准 A-Main 產出的 Issue 清單與優先級 | — |
| **開發進行中** | 監控進度、處理 AI 無法解決的環境/依賴問題 | Review 特定領域 PR (如安全性) |
| **Milestone 驗收** | **唯一有權決定是否進入下一個 Milestone** | 協助驗收 UX/安全/性能 |
| **上線決策** | 最終 Go/No-Go 決策 | 確認上線檢查清單 |
...
|--------|------|------|---------|---------|--------|
| **M1** | 基礎架構與認證 | 3 天 | DB Schema、前後端骨架、認證 API 與頁面 | A-Main, A-Backend, A-Frontend | H-Director |
...
| **M4** | 品質與自動化部署 | 5 天 | 單元/E2E 測試、CI/CD、Docker 化 | A-Main, A-QA, A-DevOps | H-Director, H-Reviewer |
...
- **A-DevOps**：產生 Dockerfile, `docker-compose.yml` 與 GitHub Actions Workflows。
...
| T-104 | 認證 API 開發 | P0 | A-Backend | ~4 AI Sessions |
| T-105 | 登入頁與前端 Auth | P0 | A-Frontend | ~3 AI Sessions |
| T-201 | 菜單與訂單 API | P0 | A-Backend | ~8 AI Sessions |
| T-202 | 顧客端訂單 UI | P0 | A-Frontend | ~10 AI Sessions |
| T-203 | 訂單狀態機 Review | P1 | A-Main | ~2 AI Sessions |
| ⛳ M2 | M2 驗收門 | P0 | H-Director | ~3 HRH |
| T-301 | 後台資料與分析 API | P1 | A-Backend | ~6 AI Sessions |
| T-302 | 管理後台 UI 與圖表 | P1 | A-Frontend | ~8 AI Sessions |
| ⛳ M3 | M3 驗收門 | P0 | H-Director, H-Reviewer | ~3 HRH |
| T-401 | 完整 CI 與部署配置 | P0 | A-DevOps | ~4 AI Sessions |
| T-402 | E2E 測試與覆蓋率 | P0 | A-QA | ~5 AI Sessions |
| T-403 | 全局整合與 Vibe Check | P0 | A-Main | ~5 AI Sessions |
| ⛳ M4 | M4 驗收門 (Go/No-Go) | P0 | H-Director, H-Reviewer | ~4 HRH |
...
- **驗證**：`npx prisma migrate dev` 成功執行無錯誤；Seed 資料可正確寫入且可查詢。
...
- **驗證**：`npm run dev` 啟動成功；`curl localhost:PORT/health` 回傳 200。
...
- **驗證**：`npm run dev` 啟動成功；瀏覽器可訪問各空白路由頁面無錯誤。

**⛳ M1 驗收門**
- **任務描述**：審查前後端專案架構、目錄結構、中介軟體實作是否符合規格預期。確認前後端可獨立啟動。
- **前置任務**：T-101, T-102, T-103
- **輸入**：T-101~T-103 的產出物
- **產出**：(無，決定是否進入 M2)
- **驗證**：H-Director 核准後方可進入下一階段。
...
- **驗證**：所有 Auth 相關單元測試通過；可透過 API 取得有效 JWT Token。

**T-105：登入頁與前端 Auth** `P0`
...
- **驗證**：登入頁可正常渲染；使用 Mock API 可模擬登入/登出流程；未登入時自動導向登入頁。
...
- **驗證**：所有菜單與訂單 API 單元測試通過；訂單狀態轉換符合狀態機定義；非法狀態轉換被正確拒絕。
...
- **驗證**：完整顧客端流程可操作 (使用 Mock API)；購物車狀態刷新後仍保留；結帳單列印樣式正確。
...
- **輸入**：T-201~T-203 的產出物
- **產出**：(無，決定是否進入 M3)
- **驗證**：H-Director 實際操作完整訂單流程無錯誤後核准。
...
- **驗證**：所有管理 API 單元測試通過；軟刪除後歷史訂單引用不受影響；聚合查詢結果正確。
...
- **驗證**：所有 CRUD 操作 UI 可正常執行 (使用 Mock API)；圖表可正確渲染測試資料；QR Code 可生成並下載。
...
- **輸入**：T-301~T-302 的產出物
- **產出**：(無，決定是否進入 M4)
- **驗證**：H-Director 驗收功能完整性；H-Reviewer 驗收 UX 設計是否符合 PRD。核准後進入 M4。

#### Milestone 4：品質與自動化部署 (G5)

**T-401：完整 CI 與部署配置** `P0`
- **任務描述**：建立完整的容器化與 CI/CD 配置：多階段 Dockerfile (前端 + 後端)、`docker-compose.yml` (含 PostgreSQL + Redis)、GitHub Actions Workflows (Lint → Type Check → Test → Build)、DB Migration 自動化腳本。
- **前置任務**：所有開發任務 (T-101 ~ T-302)
- **輸入**：專案結構、`package.json` 依賴清單
- **產出**：`Dockerfile`、`docker-compose.yml`、`.github/workflows/*.yml`、部署說明文件
- **驗證**：`docker-compose up` 可啟動完整服務棧；GitHub Actions Pipeline 全部 Green。
...
- **驗證**：`tsc --noEmit` 零錯誤；`npm run build` 前後端皆成功；所有 CI Gates 通過。
...
- **產出**：(無，Go/No-Go 決策)
- **驗證**：H-Director 確認所有檢查清單項通過；H-Reviewer 確認安全審計無高危問題。Go 則上線。
...
flowchart TB
    ...
    T101 & T102 & T103 --> M1G["⛳ M1 驗收門<br/>(H-Director)"]

    M1G --> T104 & T105

    T104["T-104 認證 API<br/>(A-Backend)"]
    T105["T-105 登入頁 UI<br/>(A-Frontend)"]
    ...
    T401["T-401 CI 與部署配置<br/>(A-DevOps)"]
...
- [ ] T-104 (A-Backend) 認證 API 開發與單元測試
- [ ] T-105 (A-Frontend) 實作 Login UI 與 API 串接
