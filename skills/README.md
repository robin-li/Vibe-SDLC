...
- **AI 職責**：同步工作目錄 → 領取 Issue（發佈 Issue Comment）→ 建立 feature 分支 → 實作 → 撰寫測試 → 遇到問題優先自行調查與解決 → Vibe Check 通過後**立即自動推送並建立 PR** → 發佈完成 Comment → 彙報結果與 PR 連結
...
- **聯調模式**：部署後的連續調試循環中（部署 → 測試 → 回報 → 修正 → 重新部署），每筆回報觸發「即時回報分流」機制（直接修 / 建 Issue / 暫存），支援累積修正後統一部署
...
- **完成條件**：CI 綠燈、Code Review 核准、PR 合併、Dev Plan 更新並推送、Issue 完成 Comment 已發佈
...
- **AI 職責**：確認部署狀態（分別檢查開發任務、手動驗證、驗收門三類 Issue）、整理回饋、根據指示更新 PRD、Dev Plan 及其他受影響的規格文件（SRD、API Spec 等）、確認版本修訂記錄已同步、比對規格文件與程式碼差異並等待開發者確認後更新
- **驗收聯調模式**：驗收期間的聯調循環中（部署 → 測試 → 回報 → 修正 → 重新部署），每筆回報觸發「即時回報分流」機制；驗收結束時主動提醒暫存 Issues、列出修正摘要、整理回饋
- **GitHub Release 流程**：驗收通過後主動引導發佈 Release（確認版本號、類型、Release Notes 風格），透過 `gh release create` 建立
- **產出**：里程碑完成確認報告、迭代回饋整理、GitHub Release（選用）
- **完成條件**：部署成功、驗收通過、回饋已反映至規格文件、所有被修改的規格文件皆已更新版本修訂記錄、GitHub Release 已發佈（若選擇發佈）
