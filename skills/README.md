...
```
> /vibe-sdlc-pr
> PR #34 的 CI 掛了，幫我看一下並修正。
```

AI 會自動抓取 CI 錯誤訊息（`gh run view`）、歸因（flaky / 既有 bug / 本次造成）後再決定處理方式，無需手動貼 log。
...
```
> /vibe-sdlc-release
> M1 驗收完了，有一些回饋需要整理，然後發佈 Release。
```

AI 會引導你逐項回報、整理為結構化回饋報告（需求變更 / 新增任務 / 不處理），確認後同步更新 PRD、Dev Plan 與版本記錄，最後發佈 Release Notes。
...
```
> /vibe-sdlc-pr
> PR #34 的 CI 通過了，A-Main 幫我做初審：確認 PR 範圍只動 /backend/**，
> 通過的話列出檢查清單給我做最終 Review。
```
