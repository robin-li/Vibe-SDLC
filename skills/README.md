...
- **Sub Agent 情境**：每個 Sub Agent 獨立 session、獨立 Worktree、最小 context 原則（僅讀取任務所需規格）、跨代理透過 GitHub Comments 溝通、操作範圍限於負責目錄；A-Main 統一批量推送與建立 PR
...
- **AI 職責**：監控 CI 結果、CI 失敗時先同步工作目錄再修正程式碼並重新提交、Merge 後同步工作目錄（`git fetch origin && git rebase origin/main`）並更新 Dev Plan 後推送、提醒手動驗證
...
- **完成條件**：CI 綠燈、Code Review 核准、PR 合併、Dev Plan 更新並推送
