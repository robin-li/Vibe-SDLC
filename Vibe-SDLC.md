...
當開發者在**部署後進行聯合調試**（部署 → 測試 → 發現問題 → 修正 → 重新部署）的連續循環中，每一筆回報的 Bug 或修改需求都必須先觸發「即時回報分流」機制（定義於核心原則第 6 條）。
...
| 2 | **開發者** | 在測試環境進行驗收測試（可進入聯調模式） | 驗收結果 |
...
在 Phase 5 驗收測試期間，開發者會在測試環境中進行聯調（部署 → 測試 → 發現問題 → 修正 → 重新部署）。此過程中每一筆回報都必須觸發「即時回報分流」機制（定義於核心原則第 6 條）：
