...
## 審查範圍
- 比對文件：01-1-PRD.md, 01-2-SRD.md, 01-5-API_Spec.md, 01-3-SDD.md, 02-Dev_Plan.md, ...
...
`/docs/status/` 下的檔案更新頻率高於一般規格文件，若「全都 commit」會淹沒 commit 歷史，若「全都忽略」則無法跨機器／跨 worktree 恢復狀態。**每個啟用 Vibe-SDLC 的專案必須從下列三種模式中選定一種，並寫入專案 `CLAUDE.md`**：

| 模式 | 適用情境 | `.gitignore` 設定 |
|------|---------|-------------------|
| **A：全版控** | 多人協作、重視完整歷史追溯 | 無需額外設定；約定狀態更新獨立 commit |
| **B：全忽略** | 單人單機、STATUS 純本地即時快照 | `docs/status/` |
| **C：混合**（推薦預設） | 單人多機、希望 STATUS 跨機器但不要 Agent 細節噪音 | `docs/status/A-*.md`（僅 `STATUS.md` 進版控） |
...
若專案 `CLAUDE.md` 未宣告模式，A-Main 在首次執行 `/vibe-sdlc-status` 時應**主動詢問使用者選擇**，並協助寫入 `CLAUDE.md` 與設定對應的 `.gitignore`，建立基準後再開始彙整。完整說明詳見 `.claude/skills/vibe-sdlc-status/skill.md`。
