...
在 Phase 5 驗收測試期間，開發者會在測試環境中進行聯調（部署 → 測試 → 發現問題 → 修正 → 重新部署）。此過程中每一筆回報都必須觸發「議題收集與處置流程」機制（定義於核心原則第 6 條）：

- **選項 1**：直接修正後詢問是否重新部署
- **選項 2**：建立 Issue（含重現步驟、截圖、標籤）→ 修正 → 詢問是否重新部署
- **選項 3**：暫存至待建立清單，繼續驗收
- **選項 4**：結束收集，批量建立 Issues 後立即進入 P3 開發流程
- **選項 5**：結束收集，批量建立 Issues 後等待開發者指示
