...
開發者（導演）負責規格定義與關鍵決策；AI 助手（執行者）負責實作、測試與報告，遇到問題優先自行調查與解決；GitHub 負責版本管控、CI/CD 與任務追蹤。
...
專案啟動
  └→ Phase 1: /vibe-sdlc-p1-spec     撰寫規格 → 交叉比對審查 → 定稿
      └→ Phase 2: /vibe-sdlc-p2-issues  審核計畫 → 建立 GitHub Issues
          └→ Phase 3: /vibe-sdlc-p3-dev      領取 Issue → 開發 → Vibe Check → 自動建 PR
              └→ Phase 4: /vibe-sdlc-p4-pr       監控 CI → 修正失敗 → Merge → 更新 Dev Plan
...
| 3 | 開發循環 | `/vibe-sdlc-p3-dev` | 日常開發，領取 Issue → 實作 → Vibe Check → 自動建 PR |
| 4 | CI 監控與合併後作業 | `/vibe-sdlc-p4-pr` | 監控 CI → 修正失敗 → Merge 後更新 Dev Plan |
...
- [`Vibe-SDLC.md`](./Vibe-SDLC.md)：完整 SOP 規範文件（v8.0）
