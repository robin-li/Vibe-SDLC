...
> **💡 空資料夾也能直接用**：若工作目錄是全新的空資料夾，`/vibe-sdlc-spec` 會進入互動式 Bootstrap 流程，協助你一次建立 `git init`、`CLAUDE.md`、`.gitignore`、`README.md`、`/docs` 骨架與 A-Main 快照分支 `dev/main-agent`，不需要事先手動初始化任何檔案。
...
- **分支策略** — ⛔ 嚴禁直接 push main。有 Issue → `feat/<agent>/<issue-N>-<簡述>`；無 Issue 小修 → `chore/main-agent/<YYYYMMDD>-<簡述>`（短命）。`dev/main-agent` 是 A-Main 快照分支，由 `/vibe-sdlc-status` 管理，**不承接工作 commit**
