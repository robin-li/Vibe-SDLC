...
```
> /vibe-sdlc-pr
> PR #34 的 CI 挂了，帮我看一下并修正。
```

AI 会自动抓取 CI 错误信息（`gh run view`）、归因（flaky / 既有 bug / 本次造成）后再决定处理方式，无需手动贴 log。
...
```
> /vibe-sdlc-release
> M1 验收完了，有一些反馈需要整理，然后发布 Release。
```

AI 会引导你逐项回报、整理为结构化反馈报告（需求变更 / 新增任务 / 不处理），确认后同步更新 PRD、Dev Plan 与版本记录，最后发布 Release Notes。
...
```
> /vibe-sdlc-pr
> PR #34 的 CI 通过了，A-Main 帮我做初审：确认 PR 范围只动 /backend/**，
> 通过的话列出检查清单给我做最终 Review。
```
