...
> **💡 空文件夹也能直接用**：若工作目录是全新的空文件夹，`/vibe-sdlc-spec` 会进入互动式 Bootstrap 流程，协助你一次建立 `git init`、`CLAUDE.md`、`.gitignore`、`README.md`、`/docs` 骨架与 A-Main 快照分支 `dev/main-agent`，无需事先手动初始化任何文件。
...
| 文件 | 路径 | 说明 |
|------|------|------|
| Docs Index | `/docs/00-Docs_Index.md` | 文件入口与导航：列出所有规格文件清单、创建时机、依赖关系 |
| PRD | `/docs/01-1-PRD.md` | 产品需求、用户故事、功能清单 |
| SRD | `/docs/01-2-SRD.md` | 非功能性需求、部署要求、安全与性能约束 |
| SDD | `/docs/01-3-SDD.md` | 系统架构、组件设计、数据模型、技术决策记录（ADR） |
| GDD | `/docs/01-4-GDD.md` | 游戏设计文件：核心机制、数值设计、关卡设计（领域选用） |
| API Spec | `/docs/01-5-API_Spec.md` + `API_Spec.yaml` | API 端点说明 + OpenAPI 合约 |
| UI/UX 设计 | `/docs/01-6-UI_UX_Design.md` + `/docs/ui/*.html` | 视觉与交互设计规格（选用）+ HTML + Tailwind wireframe 视觉参考；遵循 [UI_UX_Writing_Guidelines](./skills/vibe-sdlc-spec/references/UI_UX_Writing_Guidelines.md) 9 条准则 |
...
各 Agent 维护自己的状态档（`/docs/status/A-*.md`），A-Main 汇整为全局 `STATUS.md`。每个项目**必须**从三种版控模式中选定一种并写入 `CLAUDE.md`：

| 模式 | 适用情境 |
|------|---------|
| **A：全版控** | 多人协作、重视完整历史追溯 |
| **B：全忽略** | 单人单机、STATUS 纯本地即时快照 |
| **C：混合**（推荐预设） | 单人多机、仅 `STATUS.md` 进版控 |

> 详见 [Vibe-SDLC.md §9.4](./Vibe-SDLC.md#94-statusmd-版控策略)。若未宣告，A-Main 首次执行 `/vibe-sdlc-status` 时会主动询问并协助配置。
...
- **分支策略** — ⛔ 严禁直接 push main。有 Issue → `feat/<agent>/<issue-N>-<简述>`；无 Issue 小修 → `chore/main-agent/<YYYYMMDD>-<简述>`（短命）。`dev/main-agent` 是 A-Main 快照分支，由 `/vibe-sdlc-status` 管理，**不承接工作 commit**
...
### 场景七：测试/验收中回报 Bug 或新增需求 — 议题收集与处置流程

当你在开发、测试或验收过程中**直接描述 Bug、新需求或改善建议**时（例如「XX 坏了」「我想要 XX」），AI 会立刻停下、不动任何文件，先给你 5 个处理选项。这是 Vibe-SDLC 的核心机制（核心原则第 6 条），会在 Phase 3、4、5 任何阶段被触发。

**情境：M1 验收期间发现问题与新需求**

```
> 我刚在测试环境试了注册流程，发现密码长度只检查到 6 位，应该至少 8 位。
> 另外列表页加载有点慢，希望加上分页功能。
```
...
─────────────────
您回报了：
  - [Bug] 密码长度检查应从 6 位提升至 8 位
  - [Feature] 列表页加入分页功能

请选择处理方式：
  1️⃣  小问题，直接修正（不建 Issue）
  2️⃣  建立 Issue 后立即修正
  3️⃣  先记下来，待会一起建立 Issues
  4️⃣  议题收集完成，汇整成 Issues 并开始开发
  5️⃣  议题收集完成，汇整成 Issues 并等待指示
...
| 你的回报内容 | 建议选项 | AI 行为 |
|---|:---:|---|
| Typo、文案微调、简单 config | **1** | 在 `chore/main-agent/<date>-*` 短命分支修正并提 PR |
| 单一 Bug 或单一需求要立即处理 | **2** | 建 Issue → 立即进入 P3 开发 → 自动建 PR |
| 一连串反馈要先收集 | **3** | 暂存清单，等你说「整理回报」再批量建立 |
| 确认的清单要立刻全部开做 | **4** | 批量建 Issues → 立即依优先序开发 |
| 确认的清单要先 review 优先级 | **5** | 批量建 Issues → 停下等指示 |

**选项 4 范例对话（验收后一次性处理多项反馈）：**

```
> 4
> 第 1 项是 P0 Bug 要进 M1，第 2 项是新需求进 M2 backlog。
```

AI 会逐一确认每笔 Issue 的标题、Labels、Milestone，批量执行 `gh issue create`，接着直接领取 P0 Issue 进入 Phase 3 开发循环。若回报内容涉及 PRD / API Spec 变更，AI 会在处理完成后**即时回溯更新**对应规格文件并升版（核心原则第 4、5 条）。

> 💡 **何时不会触发**：透过 `/vibe-sdlc-dev` 领取既有 Issue、纯粹的程式码问答、或你明确说「直接改」「快速修一下」时，AI 会直接执行不再询问。

---

### 场景八：Multi Sub Agent 并行开发
