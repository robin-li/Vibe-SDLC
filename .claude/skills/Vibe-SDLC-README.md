...
| `/vibe-sdlc-spec` | Phase 1：定義規格文件與計畫 | 撰寫 / 審查 PRD、SRD、API Spec、Dev Plan，產出完整性審查報告 |
...
| `/vibe-sdlc-release` | Phase 5：交付與迭代 | 部署驗收 → 收集回饋 → 更新規格 |
...
### Phase 5：交付與迭代 (`/vibe-sdlc-release`)

里程碑完成後的**部署、驗收與回饋收集**。

- **前置條件**：所有開發任務 Issues 已合併、所有手動驗證 Issues 已由審查角色關閉（附驗證結果）、Dev Plan 任務已標記完成
- **AI 職責**：確認部署狀態（分別檢查開發任務、手動驗證、驗收門三類 Issue）、整理回饋、根據指示更新 PRD、Dev Plan 及其他受影響的規格文件（SRD、API Spec 等）、確認版本修訂記錄已同步
- **產出**：里程碑完成確認報告、迭代回饋整理
- **完成條件**：部署成功、驗收通過、回饋已反映至規格文件、所有被修改的規格文件皆已更新版本修訂記錄
