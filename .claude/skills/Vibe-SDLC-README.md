...
- **AI 職責**：同步工作目錄 → 讀取 Issue → 建立 feature 分支 → 實作 → 撰寫測試 → Vibe Check 通過後**立即自動推送並建立 PR** → 彙報結果與 PR 連結
...
AI 完成開發與測試後，Vibe Check 通過會**立即自動推送分支並建立 PR**，然後向你彙報結果與 PR 連結。你只需在 GitHub 上進行 Code Review。
