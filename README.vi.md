...
> **💡 Thư mục trống cũng dùng được ngay**: Nếu thư mục làm việc là thư mục trống hoàn toàn mới, `/vibe-sdlc-spec` sẽ bước vào luồng Bootstrap tương tác, giúp bạn tạo `git init`, `CLAUDE.md`, `.gitignore`, `README.md`, khung `/docs` và nhánh thường trú `dev/main-agent` trong một lần — không cần khởi tạo thủ công trước.
