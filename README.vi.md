...
> **💡 Thư mục trống cũng dùng được ngay**: Nếu thư mục làm việc là thư mục trống hoàn toàn mới, `/vibe-sdlc-spec` sẽ bước vào luồng Bootstrap tương tác, giúp bạn tạo `git init`, `CLAUDE.md`, `.gitignore`, `README.md`, khung `/docs` và nhánh snapshot A-Main `dev/main-agent` trong một lần — không cần khởi tạo thủ công trước.
...
- **Chiến lược branch** — ⛔ Nghiêm cấm push trực tiếp vào main. Có Issue → `feat/<agent>/<issue-N>-<mô-tả>`; Không có Issue (sửa nhỏ) → `chore/main-agent/<YYYYMMDD>-<mô-tả>` (ngắn hạn). `dev/main-agent` là nhánh snapshot A-Main do `/vibe-sdlc-status` quản lý, **không nhận work commit**
...
|---|:---:|---|
| Typo, chỉnh copy, config đơn giản | **1** | Sửa trên nhánh ngắn hạn `chore/main-agent/<date>-*` và tạo PR |
...
├── Vibe-SDLC.md                       ← Tài liệu SOP đầy đủ (v8.8)
...
| Tài liệu | Mô tả |
|-----------|-------|
| [`Vibe-SDLC.md`](./Vibe-SDLC.md) | Tài liệu SOP đầy đủ (v8.8) với mọi chi tiết quy trình |
